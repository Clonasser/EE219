...
void vsup_vv(register int* vd, register int* rs1){
    asm volatile( ".insn r 0x57, 0x0, 0x1f, %0, %1, x0": :"r"(vd), "r"(rs1));
}
...
int main(){
    ...
    for(i=0; i<EMBED_DIM; i++){
        ...
        for(j=0; j<FEAT_ROW_NUM; j++){
            ...
            for(k=0; k<12; k++){// 12 = 192/16 = FEAT_COL_NUM / valu element's num 
            ...
            vsup_vv(reg_vx1,reg_vx2);
            vse32(reg_vx1, (int *)(ADDR_PAR_SUM));
            vle32(reg_vx2, (int *)(ADDR_PAR_SUM+64));
            vsup_vv(reg_vx1,reg_vx2);
            vse32(reg_vx1, (int *)(ADDR_PAR_SUM+8));
            vle32(reg_vx2, (int *)(ADDR_PAR_SUM));
    ...
    for(h=0; h<HEAD_NUM; h++){
        for(qr=0; qr<FEAT_ROW_NUM; qr++){
            for(kr=0; kr<FEAT_ROW_NUM; kr++){
                vse32(reg_vx0, (int *)(ADDR_PAR_SUM));
                for(head_split=0; head_split<4; head_split++){ //4 = 64/16 = head's EDIM / valu element's num 
                    vle32(reg_vx1,(int *)(ADDR_QSAVE + qr*EMBED_DIM*4 + h*64*4 + head_split*16*4));
                    vle32(reg_vx2,(int *)(ADDR_KSAVE + kr*EMBED_DIM*4 + h*64*4 + head_split*16*4));
                    vmac_vv(reg_vx1, reg_vx1, reg_vx2);
        ...
        for(vc=0; vc<64; vc++){ // 64 = EDIM / HEAD_NUM
            vse32(reg_vx0, (int *)(ADDR_QSAVE));
            for(k=0; k<FEAT_ROW_NUM; k++){
                p_VT[k] = p_VSAVE[k*EMBED_DIM + h*64 + vc];
            }
            // for(k=0; k<16; k++){
            //     printf("%d\t",p_VT[k]);
            // }
            ...
            for(qkr=0; qkr<FEAT_ROW_NUM; qkr++){
                ...
                vse32(reg_vx0, (int *)(ADDR_KSAVE));
                vle32(reg_vx2, (int *)(ADDR_QSAVE+64));
                for(k=0; k<16; k++){
                    p_ATemp[k] = p_A[k + qkr*FEAT_ROW_NUM +64];
                }
                vle32(reg_vx1, (int *)(ADDR_KSAVE));
                ...
                vse32(reg_vx0, (int *)(ADDR_KSAVE));
                vle32(reg_vx2, (int *)(ADDR_QSAVE+128));
                for(k=0; k<16; k++){
                    p_ATemp[k] = p_A[k + qkr*FEAT_ROW_NUM +128];
                }
                vle32(reg_vx1, (int *)(ADDR_KSAVE));

                vmac_vv(reg_vx1, reg_vx1, reg_vx2);
                vse32(reg_vx1, (int *)(ADDR_PAR_SUM+16));

                vse32(reg_vx0, (int *)(ADDR_KSAVE));
                vle32(reg_vx2, (int *)(ADDR_QSAVE+192));
                for(k=0; k<2; k++){
                    p_ATemp[k] = p_A[k + qkr*FEAT_ROW_NUM +192];
                }
                vle32(reg_vx1, (int *)(ADDR_KSAVE));

                vmac_vv(reg_vx1, reg_vx1, reg_vx2);
                vse32(reg_vx1, (int *)(ADDR_PAR_SUM+24));

                vle32(reg_vx1, (int *)(ADDR_PAR_SUM));

                vsum_vv(reg_vx1,reg_vx1);
                p_SAVE[h*64 + vc + qkr * EMBED_DIM]=p_PARSUM[0];
